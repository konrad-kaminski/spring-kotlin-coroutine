 ...
 * @since 4.2
 */
public abstract class AbstractCoroutineApplicationListenerMethodAdapter implements GenericApplicationListener {
    ...
    public AbstractCoroutineApplicationListenerMethodAdapter(String beanName, Class<?> targetClass, Method method) {
    ...
    private List<ResolvableType> resolveDeclaredEventTypes(Method method, EventListener ann) {
        ...
        if (count > 2) {
            throw new IllegalStateException(
                    "Maximum one parameter is allowed for suspending event listener method: " + method);
        }
    ...
    abstract protected void processEvent(ApplicationEvent event);
